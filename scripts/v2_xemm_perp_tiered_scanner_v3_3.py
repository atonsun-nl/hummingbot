...
class XEMMPerpTieredScannerConfig(StrategyV2ConfigBase):
    ...
    min_holding_time: int = Field(
        default=120,
        client_data=ClientFieldData(
            prompt=lambda e: "Min holding time (sec) before rotation: ",
            prompt_on_new=True,
        ))
...
class XEMMPerpTieredScanner(StrategyV2Base):
    ...
    # --------------------------------------------- completed hedge closer
    def _close_completed_hedges(self) -> None:
        now = self.current_timestamp
        executors = self.get_all_executors()
        if executors and self._tick_counter % 30 == 0:
            for e in executors:
                ct = getattr(e, "close_type", None)
                self.logger().debug(
                    f"[CLOSE-CHECK] executor={str(e.id)[:8]} "
                    f"status={e.status} close_type={ct} "
                    f"is_done={e.is_done} "
                    f"type={getattr(e.config, 'type', '?')}"
                )
        for e in executors:
            if str(e.id) in self._closed_executor_ids:
                continue
            ct = getattr(e, "close_type", None)
            if ct != CloseType.COMPLETED:
                continue
            if not e.is_done:
                continue
            created = float(getattr(e.config, "timestamp", 0) or 0)
            holding_sec = now - created if created > 0 else 0
            if holding_sec < self.config.min_holding_time:
                continue

            pair = e.config.buying_market.trading_pair
            buying_cn = e.config.buying_market.connector_name
            selling_cn = e.config.selling_market.connector_name
            buying_connector = self.connectors.get(buying_cn)
            selling_connector = self.connectors.get(selling_cn)
            if buying_connector is None or selling_connector is None:
                continue

            amount = e.config.order_amount
            try:
                # buying_market always holds LONG → close with sell(CLOSE)
                buying_connector.sell(
                    pair, amount,
                    order_type=OrderType.MARKET,
                    position_action=PositionAction.CLOSE,
                )
                # selling_market always holds SHORT → close with buy(CLOSE)
                selling_connector.buy(
                    pair, amount,
                    order_type=OrderType.MARKET,
                    position_action=PositionAction.CLOSE,
                )
                self._closed_executor_ids.add(str(e.id))
                self.logger().info(
                    f"[CLOSE] Closing hedged positions for {pair} "
                    f"(held {holding_sec:.0f}s): "
                    f"LONG on {buying_cn} + SHORT on {selling_cn}, "
                    f"amount={amount}"
                )
            except Exception as ex:
                self.logger().warning(f"[CLOSE] Failed to close {pair}: {ex}")
