...
class XEMMPerpTieredScanner(StrategyV2Base):
    ...
    def __init__(self, connectors: Dict[str, ConnectorBase], config: XEMMPerpTieredScannerConfig):
        ...
        # Lock for protecting shared resources in REST scan loop
        self._rest_cache_lock = asyncio.Lock()
    ...
    def _apply_position_mode_only(self):
        for connector_name in (self.config.maker_connector, self.config.taker_connector):
            ...
            if connector is None:
                continue
            if not self._is_perpetual(connector_name):
                continue
    ...
    def _validate_perp_setup(self) -> None:
        ...
        for cn in (self.config.maker_connector, self.config.taker_connector):
            ...
            try:
                # Check if position_mode attribute exists before accessing it
                if hasattr(connector, "position_mode"):
                    ...
                    problems.append(f"{cn}: missing position_mode attribute")
    ...
    async def _safe_async_task(self, coro, task_name: str = "unknown"):
        """Wrapper to catch and log unhandled exceptions in async tasks."""
        try:
            return await coro
        except asyncio.CancelledError:
            self.logger().debug(f"[ASYNC-TASK] {task_name} cancelled")
            raise
        except Exception as e:
            self.logger().error(f"[ASYNC-TASK] {task_name} failed with exception: {e}", exc_info=True)
            raise
    ...
    def _set_margin_mode_for_pair(self, trading_pair: str):
        for connector_name in (self.config.maker_connector, self.config.taker_connector):
            ...
            if margin_mode_enum is None:
                continue
            # Wrap async task with exception handling to prevent unhandled exceptions
            asyncio.ensure_future(
                self._safe_async_task(
                    self._set_single_pair_margin_mode_bitget(
                        connector, connector_name, trading_pair, margin_mode_enum
                    ),
                    f"_set_margin_mode_for_pair:{connector_name}:{trading_pair}"
                )
            )
    ...
    async def _fetch_rest_tickers(self):
        ...
        # Use lock to protect shared cache update
        async with self._rest_cache_lock:
    ...
    def on_tick(self):
        ...
        if self._rest_universe and AIOHTTP_AVAILABLE:
            ...
            if (
                ...
                self._rest_scan_task = asyncio.ensure_future(self._safe_async_task(self._rest_scan_loop(), "_rest_scan_loop"))
        ...
        if self._pending_ob_subscriptions and (
            ...
            self._ob_subscribe_task = asyncio.ensure_future(self._safe_async_task(
                self._subscribe_orderbooks(pending), f"_subscribe_orderbooks:{len(pending)}_pairs")
            )
    ...
    def _build_executor_action(
        ...
        config = XEMMExecutorConfig(
            timestamp=float(self.current_timestamp),  # Ensure float type for timestamp
